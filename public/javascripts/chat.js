...
$(document).ready(function()
{
  ...
  presenceChannel.bind('pusher:member_added', function(member) {
    $('#messages').append('<li class="note"><strong>' + member.chat_user.nickname + '</strong> 加入.</li>');
  ...
  presenceChannel.bind('pusher:member_removed', function(member) {
    $('#messages').append('<li class="note"><strong>' + member.chat_user.nickname + '</strong> 离开.</li>');
    $("#messages").attr("scrollTop",$("#messages").attr("scrollHeight") - $('#messages').height());
    updateCount(-1);
  });
  ...
  presenceChannel.bind('updated_nickname', function(member) {
    ...
    {
      $('#messages').append('<li class="note">你已经更新昵称为: <strong>' + member.nickname + '</strong>.</li>');
    }
    else
    {
      $('#messages').append('<li class="note"><strong>' + member.old_nickname + '</strong> 更新昵称为: <strong>' + member.nickname + '</strong>.</li>');
    }
    $("#messages").attr("scrollTop",$("#messages").attr("scrollHeight") - $('#messages').height());
  });
  ...
  presenceChannel.bind('send_message', function(message) {
    ...
    $('#messages').append('<li class="' + you + 'just_added_id_' + message.id + '" style="display:none;"><strong>' + message.user.nickname + '</strong> 说:<br />' + replaceURLWithHTMLLinks(message.message) + '</li>');
  ...
  presenceChannel.bind('typing_status', function(notification) {
    ...
    if(notification.status == "true") {
      $('#messages').append('<li class="note" id="is_typing_'+ notification.user.id +'"><strong>' + notification.user.nickname + '</strong> 正在输入...</li>');
    } else {
